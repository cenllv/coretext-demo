...
- (void)buildFrames {
    ...
    while (textPos < [attString length]) { //4
        ...
        CTColumnView *columnView = [[CTColumnView alloc] initWithFrame:CGRectMake(0, 0, self.contentSize.width, self.contentSize.height)];
        columnView.backgroundColor = [UIColor clearColor];
        columnView.frame = CGRectMake(colOffset.x, colOffset.y, colRect.size.width, colRect.size.height) ;
